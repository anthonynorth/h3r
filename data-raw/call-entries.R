...
defs <- tibble(
  def = src_sources %>%
    str_extract_all(regex("SEXP h3r_c_[^\\)]+\\)\\s+", multiline = TRUE)) %>%
    unlist() %>%
    str_replace_all("\\s+", " ") %>%
    str_trim(),
  name = def %>% str_extract("h3r_c_[^\\(]+"),
  return_type = "SEXP",
  args = def %>%
    str_remove("SEXP h3r_c_[^\\(]+\\(") %>%
    str_remove("\\)$") %>%
    str_split("\\s*,\\s*") %>%
...
init <- read_file("src/h3r-init.c")
pattern <- regex(
  "\n/\\* generated by data-raw/call-entries\\.R \\*/.*?/\\* end generated by data-raw/call-entries\\.R \\*/",
  multiline = TRUE,
  dotall = TRUE
)
